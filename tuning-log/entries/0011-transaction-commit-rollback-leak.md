...
### 再追記: このエントリの前提自体が誤りだった(重要)

その後`main.ts`を確認したところ、全リクエストを包む最上位ミドルウェア(`app.use(...)`、47行目付近)が
以下のように実装されていることが判明した:
...
**ハンドラの実行後、必ずこの`finally`で`connection.rollback()`が呼ばれてからコネクションが
プールに返却される。** つまり各ハンドラ内で`commit()`/`rollback()`をどちらも呼ばずに早期returnしても、
この安全網によってトランザクションは常に閉じられてからプールに戻っていた。「仮説」節で立てた
「commit/rollback漏れでコネクションプール上にトランザクションが未終了のまま残り、ロック競合や
502エラーを引き起こしている」という前提そのものが誤りだった。

この発見を踏まえると:

- 「変更」節で追加した明示的な`rollback()`呼び出しは、既存の安全網に対して**冗長**(2重にROLLBACKを
  送るだけ)であり、実害はないがバグ修正でもなかった。追記(1回目)で見た「ROLLBACK回数がbefore→after
  で32702→35586に増えた」という現象も、単に追加した分だけROLLBACK文が余分に発行されるようになった
  だけで完全に説明がつく。
- `appGetNotification`の`beginTransaction()`への`await`追加は、同一コネクション上のクエリが
  発行順に処理される(mysql2の単一コネクションはコマンドをキューイングして順次処理する)ため、
  実質的な影響はごく小さいと考えられる。
- したがって「検証」節で見たスコア改善(中央値 3905→4322、約+10.7%)は、**この修正の効果ではなく
  計測ノイズの範囲内である可能性が高い**。before(3885〜4912)とafter(3956〜5086)のレンジは
  大きく重なっており、3回計測程度ではこの規模の差を有意と判断できない。

**結論**: 今回の変更(冗長なrollback呼び出しの追加)自体は無害だが、当初の仮説("接続プール上の
トランザクションリークが不安定さの原因")は誤りであり、観測したスコア改善もこの変更に起因すると
主張することはできない。次にこの種のロック競合仮説を検証する際は、まずミドルウェア層まで含めた
コネクションのライフサイクル全体を確認してから仮説を立てる。不安定さの真因は依然として未解明のまま
残っている。
